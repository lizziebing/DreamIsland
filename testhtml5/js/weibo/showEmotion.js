...
// 初始化缓存，页面仅仅加载一次就可以了
$(function() {
    var dataroot="http://42.121.125.50/face_resource/emotions.json"; 
    $.getJSON(dataroot, function(data){ 
        for ( var i in data) {
		    if (data[i].category == '') {
			    data[i].category = '常用表情';
			}
...
(function($){
	$.fn.SinaEmotion = function(target){
		...
		$(this).click(function(event){
			...
			$('#sendHandle').append('<div id="emotions"></div>');
		//	$('#emotions').css({top: eTop, left: eLeft});
			...
			$('#emotions').html('<div style="float:right" ><a href="javascript:void(0);" id="close"  >&times;</a></div><div class="categorys"></div><div class="container"></div><div class="page"></div>');
		    $('#emotions #close').click(function(){     
			    $('#emotions').remove();
		    });
		...
		function showEmotions(){
			var category = arguments[0]?arguments[0]:'常用表情';
